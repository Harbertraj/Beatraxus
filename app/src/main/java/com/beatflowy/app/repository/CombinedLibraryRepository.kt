...
class CombinedLibraryRepository(
    private val allSongs: Flow<List<Song>>
) : LibraryRepository {

    override fun getSongs(): Flow<List<Song>> = allSongs

    override fun getAlbums(): Flow<List<Triple<String, String, Uri?>>> = allSongs.map { songs ->
        songs.groupBy { it.album }
            .map { (name, list) -> Triple(name, list.first().artist, list.first().albumArtUri) }
            .sortedBy { it.first.lowercase() }
    }

    override fun getArtists(): Flow<List<Triple<String, String, Uri?>>> = allSongs.map { songs ->
        songs.groupBy { it.artist }
            .map { (name, list) -> Triple(name, "${list.size} songs", list.first().albumArtUri) }
            .sortedBy { it.first.lowercase() }
    }

    override fun getYears(): Flow<List<Triple<String, String, Uri?>>> = allSongs.map { songs ->
        songs.groupBy { it.year }
            .map { (year, list) -> Triple(year.toString(), "${list.size} songs", list.first().albumArtUri) }
            .sortedByDescending { it.first }
    }

    override fun getGenres(): Flow<List<Triple<String, String, Uri?>>> = allSongs.map { songs ->
